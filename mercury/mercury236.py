...
def send_tcp_command(s, address_mercury, command, *params, **kwargs):
    address = pack('!B', address_mercury)
    params = bytes(params)
    ...
    if kwargs.get('crc', True):
        message += modbus_crc(message.decode('latin1')).encode('latin1')
    s.sendall(message)
    ...
    while True:
        ...
        try:
            ...
            if data:
                ...
                break  # Если данные не были получены, прерываем цикл
        except socket.timeout:
            break  # Прерываем цикл, если время ожидания истекло
        except Exception as e:
            print(f"Error while reading from socket: {e}")
            break
    s.settimeout(None)
    return buffer
